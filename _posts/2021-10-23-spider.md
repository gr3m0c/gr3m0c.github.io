...
Playing around with the input certain characters and words are blocked like double braces. It seems like someone implemented a blacklist against SSTI.
...
[![blocked2](/img/spider/blocked2.png)](/img/spider/blocked2.png)
 ...
 Since the message stated the support portal should be fixed we might be lucky and the blacklist is not implemented fully. Single braces with percent and `with` seem to be working so we can adapt this [jinja2 obfuscated payload](https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/Server%20Side%20Template%20Injection#jinja2---filter-bypass) from [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings).
