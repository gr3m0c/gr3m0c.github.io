...
---
[![box_card.png](/img/tenet/box_card)](/img/tenet/box_card)

Tenet is a medium rated machine on HackTheBox created by [egotisticalSW](https://www.hackthebox.eu/home/users/profile/94858). For the user part we will find a backup php script and abuse the php unserialze function to reach RCE on the webserver resulting in a reverse shell. Next we find database credentials, with wich we can use to switch to the user Neil. Finally we will exploit a race condition in a bash script, we are able to run as root to write our public ssh key to root's authorized ssh keys.

# User

## Nmap 

We start our enumeration on the target off with a full port nmap scan, followed by a script and version scan to get a full picture of the attack surfave.
...
Version and script scan open ports
...
[![migration_blogpost.png](/img/tenet/migration_blogpost)](/img/tenet/migration_blogpost)

Navigating to `http://[machine-ip]/sator.php.bak` let's us retrive the backup of the file 

[![sator_php_bak.png](/img/tenet/sator_php_bak)](/img/tenet/sator_php_bak)
...
The retrieved code takes userinput and passes it to the unserialize function. We want to abuse this with creating our own serialized DatabaseExport instance.
...
[![burp_deserialize.png](/img/tenet/sator_php_bak)](/img/tenet/sator_php_bak)

[![rce.png](/img/tenet/sator_php_bak)](/img/tenet/sator_php_bak)

First we set up out listener to catch the reverseshell.
...
Then with the next request, we get a bash revershell in burp from our webshell, recieve a connection back on the target and upgrade the revshell.

[![burp_revshell.png](/img/tenet/sator_php_bak)](/img/tenet/sator_php_bak)
...
While the loops are running we exectue the script as root, not here that because of the nature of the race condition you might have to repeat this request. To increase the chance you could e.g run more of the above loops.
